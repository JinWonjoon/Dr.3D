...
def convert_sdf_samples_to_obj(
    ...
    :voxel_grid_origin: a list of three floats: the bottom, left, down origin of the voxel grid
    :voxel_size: float, the size of the voxels
    :obj_filename_out: string, path of the filename to save to
    This function adapted from: https://github.com/RobotLocomotion/spartan
    """
    start_time = time.time()
    ...
    verts_ = np.zeros((num_verts,3))
    for i in range(0, num_verts):
        verts_[i] = mesh_points[i, :]
        
    faces_ = np.zeros((num_faces,3))
    for i in range(0, num_faces):
        faces_[i] = faces[i, :]

    mesh = trimesh.Trimesh(vertices=verts_, faces=faces_)
    mesh.export(obj_filename_out)
    print(f"wrote to {obj_filename_out}")
