...
import torch
from src.training.networks.networks_stylegan2 import Conv2dLayer, SynthesisLayer, ToRGBLayer
from src.torch_utils.ops import upfirdn2d
from src.torch_utils import persistence
from src.torch_utils import misc

from src.training.networks.networks_stylegan2 import SynthesisBlock
import numpy as np

#----------------------------------------------------------------------------
## Can select number of upscale (karmina)
#----------------------------------------------------------------------------

@persistence.persistent_class
class SuperresolutionUpUp(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
        ...
        self.input_resolution = img_resolution // 4
        self.sr_antialias = sr_antialias
        self.block0 = SynthesisBlock(channels, 128, w_dim=512, resolution=self.input_resolution * 2,
        ...
        print(f"[Use SuperresolutionUpUp] input :{self.input_resolution}")
...
@persistence.persistent_class
class SuperresolutionNoupUp(torch.nn.Module):
    def __init__(self, channels, img_resolution, sr_num_fp16_res, sr_antialias,
                num_fp16_res=4, conv_clamp=None, channel_base=None, channel_max=None,# IGNORE
                **block_kwargs):
        super().__init__()
        ...
        self.block1 = SynthesisBlock(128, 64, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=True, use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), **block_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
        print(f"[Use SuperresolutionNoupUp] input :{self.input_resolution}")
