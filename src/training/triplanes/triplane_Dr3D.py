...
class TriPlaneGeneratorDr3D(torch.nn.Module):
    def __init__(self,
        ...
        self.z_orig_dim = z_dim
        self.z_deform_dim = additional_kwargs['deform_kwargs']['z_dim']
        self.z_dim = self.z_orig_dim + self.z_deform_dim
        # self.z_dim = z_dim
        ...
        self.deform = dnnlib.util.construct_class_by_name(**additional_kwargs['deform_kwargs'])
    ...
    def mapping(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        z_orig, z_deform = z[:, :self.z_orig_dim], z[:, self.z_orig_dim:]
        ws_orig = self.mapping_orig(z_orig, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas) # e.g. [B,512]
        ws_deform = self.mapping_deform(z_deform, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas) # e.g. [B,256]

        # ws = torch.cat((ws_orig, ws_deform), dim=1) # [batch, num_ws, w_dim_orig + w_dim_deform]
        ws = {}
        ws['ws_orig'] = ws_orig
        ws['ws_deform'] = ws_deform
        return ws
    
    def mapping_orig(self, z_wobg, c, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        if self.rendering_kwargs['c_gen_conditioning_zero']:
            c = torch.randn((c.shape[0], 0))
        return self.backbone.mapping(z_wobg, c * self.rendering_kwargs.get('c_scale', 0), truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)

    def mapping_deform(self, z_deform, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        c = torch.zeros((z_deform.shape[0], 0))
        return self.deform.mapping(z_deform, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
    ...
    def synthesis(self, ws, c, neural_rendering_resolution=None, update_emas=False, cache_backbone=False, use_cached_backbone=False, **synthesis_kwargs):
        ...
        # Create 2D feature maps for modulation (Deformation Networks in Sec.4.1)
        feature_maps = self.deform.get_deform_feature(ws['ws_deform'])
        ...
        if use_cached_backbone and self._last_planes is not None:
            ...
            planes = self.backbone.synthesis(ws['ws_orig'], feature_maps, update_emas=update_emas, **synthesis_kwargs)
        if cache_backbone:
        ...
        sr_image = self.superresolution(rgb_image, feature_image, ws['ws_orig'], noise_mode=self.rendering_kwargs['superresolution_noise_mode'], **{k:synthesis_kwargs[k] for k in synthesis_kwargs.keys() if k != 'noise_mode'})
    ...
    def get_input(self, out_dicts, training_sets, rank, batch_gpu, batch_size, phases, P):
        device = torch.device('cuda', rank)
        ## out_dicts['image'] : batch_gpu x 3 x H x W
        imgs = torch.tensor(out_dicts['image']).to(device).to(torch.float32) / 127.5 - 1
        phase_real_img = (imgs).split(batch_gpu)  # batch x 3 x H x W
        
        # get label.
        # phase_real_c = out_dicts['label'].to(device).split(batch_gpu) # batch x 25
        with torch.no_grad():
            P.eval()
            phase_real_pose, phase_real_c = P.get_pose_label(imgs)
            phase_real_pose = phase_real_pose.detach().to(device).to(torch.float32).split(batch_gpu) # batch x 3.
            phase_real_c = phase_real_c.detach().to(device).to(torch.float32).split(batch_gpu) # batch x 25.
            P.train()
        real_inputs = {
            'real_img': list(phase_real_img),
            'real_c': list(phase_real_c),
            'real_pose': list(phase_real_pose)
        }
        ...
        all_gen_c = [c.split(batch_size) for c in all_gen_c]
        gen_inputs = {
            'gen_z': list(all_gen_z),
            'gen_c': list(all_gen_c)
        }
